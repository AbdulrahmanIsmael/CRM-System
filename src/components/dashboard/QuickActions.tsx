...
export function QuickActions() {
  ...
  const actions = [
    {
      ...
      icon: ContactRound,
      tone: "bg-primary/10 text-primary-light",
    },
    {
      label: t("newDeal"),
      href: "/deals?action=new",
      icon: ListPlus,
      tone: "bg-accent/10 text-accent-light",
    },
    {
      label: t("newProject"),
      href: "/projects?action=new",
      icon: FolderPlus,
      tone: "bg-secondary/10 text-secondary-light",
    },
    {
      label: t("newInvoice"),
  ...
  return (
    <Card className="h-full min-h-80 overflow-hidden rounded-2xl border-border/80 bg-card/95 shadow-sm">
      ...
      <CardContent className="grid flex-1 grid-cols-2 gap-3 pb-5">
        {actions.map(({ label, href, icon: Icon, tone }) => (
          <Link
            key={href}
            href={href}
            className="group flex min-h-29.5 flex-col justify-between rounded-2xl border border-border/80 bg-surface/45 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:bg-surface-light/60 hover:shadow-[0_14px_34px_rgb(0_0_0/0.16)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
          >
            <span

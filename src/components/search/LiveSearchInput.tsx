...
export function LiveSearchInput({
  ...
  return (
    <div className="relative max-w-xl flex-1">
      <Search className="pointer-events-none absolute inset-s-4 top-1/2 size-4.5 -translate-y-1/2 text-muted-foreground" />
      <input
        value={query}
      ...
      {isPending ? (
        <LoaderCircle
          className="pointer-events-none absolute inset-e-10 top-1/2 size-4 -translate-y-1/2 animate-spin text-primary"
          aria-hidden="true"
        />
      ) : null}
    </div>

...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({ locale: locale as AppLocale, key: "profile", path: "/profile" });
}
...
export default async function PageLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <PageSchema locale={locale as AppLocale} path="/profile" name={t("profile.title")} description={t("profile.description")} />
      {children}
    </>
  );
}

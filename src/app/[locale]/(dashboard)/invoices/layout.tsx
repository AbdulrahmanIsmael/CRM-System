...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({ locale: locale as AppLocale, key: "invoices", path: "/invoices" });
}
...
export default async function PageLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <PageSchema locale={locale as AppLocale} path="/invoices" name={t("invoices.title")} description={t("invoices.description")} />
      {children}
    </>
  );
}

...
export default async function InvoicesPage({ searchParams }: { searchParams?: Promise<{ action?: string; q?: string }> }) {
  ...
  return <div className="space-y-6 pb-10">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary-light">Nexus CRM</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{tn("invoices")}</h1><p className="mt-2 text-sm text-muted-foreground">{t("subtitle")}</p></div><InvoiceCreateDialog contacts={contactOptions} projects={projectOptions} autoOpen={params.action === "new"}/></div>
    <form className="relative max-w-xl"><Search className="pointer-events-none absolute start-4 top-1/2 size-4.5 -translate-y-1/2 text-muted-foreground"/><Input name="q" aria-label={t("searchPlaceholder")} defaultValue={q} className="h-12 rounded-2xl ps-11" placeholder={t("searchPlaceholder")}/></form>

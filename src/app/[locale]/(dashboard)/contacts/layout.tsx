...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({ locale: locale as AppLocale, key: "contacts", path: "/contacts" });
}
...
export default async function PageLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <PageSchema locale={locale as AppLocale} path="/contacts" name={t("contacts.title")} description={t("contacts.description")} />
      {children}
    </>
  );
}

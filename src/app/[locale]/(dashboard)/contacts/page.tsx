...
import { ExternalLink, Mail, Phone, UsersRound } from "lucide-react";
import {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        />
        <div className="hidden items-center rounded-2xl border border-border bg-surface/60 px-4 text-xs text-muted-foreground sm:flex">
          <UsersRound className="me-2 size-4 text-primary-light" />
          {contacts?.length ?? 0} {t("title")}
        </div>
      </div>
      {error ? (
        <Card>
          <CardContent className="p-8 text-sm text-danger">
            {tc("retry")}
          </CardContent>
        </Card>
      ) : (contacts ?? []).length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex min-h-64 flex-col items-center justify-center p-8 text-center">
            ...
            <h2 className="mt-4 text-base font-semibold">{t("noContacts")}</h2>
        ...
        <div className="grid gap-4 xl:grid-cols-3 2xl:grid-cols-4">
          {contacts!.map((contact) => {
            const name = getContactDisplayName(contact) || tc("unknown");
            return (
              <Card
                key={contact.id}
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3">
                      <span className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-linear-to-br from-primary/25 to-accent/10 text-sm font-semibold text-primary-light">
                        {getInitials(name)}
                      </span>
                      <div className="min-w-0">
                        <Link
                          href={`/contacts/${contact.id}`}
                          className="block truncate font-semibold hover:text-primary-light"
                        >
                          {name}
                        </Link>

...
export default async function ProjectsPage({
  ...
  let projectQuery = supabase
    .from("projects")
    .select(
      "id,name,status,budget,deadline,start_date,description,contact_id,deal_id,contacts(type,first_name,last_name,company_name)",
    )
  ...
  (tasks ?? []).forEach((task) => {
    ...
    const current = taskMap.get(task.project_id) ?? {
      total: 0,
      done: 0,
    };
      ...
      {(projects ?? []).length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex min-h-64 flex-col items-center justify-center text-center">
            ...
            <h2 className="mt-4 font-semibold">{t("noProjects")}</h2>
        ...
        <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
          {projects!.map((project) => {
            ...
            const client =
              getContactDisplayName(contact || {}) || t("clientUnknown");

            const stat = taskMap.get(project.id);
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <Link
                        ...
                        className="block truncate text-base font-semibold hover:text-primary-light"
                      >
                        {project.name}
                      </Link>
                    ...
                    <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100">
                      ...
                      <DeleteEntityButton entity="project" id={project.id} />
                  ...
                  <div className="mt-5">
                    <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <span>
                        {progress === null ? t("noProgress") : `${progress}%`}
                      </span>
                    </div>

...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({ locale: locale as AppLocale, key: "projects", path: "/projects" });
}
...
export default async function PageLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <PageSchema locale={locale as AppLocale} path="/projects" name={t("projects.title")} description={t("projects.description")} />
      {children}
    </>
  );
}

...
export default async function ProjectDetailsPage({
  ...
  const [
    ...
    supabase
      .from("projects")
      .select(
        "id,name,description,status,start_date,deadline,budget,contact_id,deal_id,contacts(type,first_name,last_name,company_name),deals(title,currency)",
      )
      .eq("id", id)
      .maybeSingle(),
    supabase
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(Number(project.budget || 0), currency, locale)}
            </div>
          </CardContent>
        </Card>
        <Card>

...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({ locale: locale as AppLocale, key: "dashboard", path: "/dashboard" });
}
...
export default async function PageLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <PageSchema locale={locale as AppLocale} path="/dashboard" name={t("dashboard.title")} description={t("dashboard.description")} />
      {children}
    </>
  );
}
